    ...
    <div class="row" v-if="show">
      <div class="col-lg-12">
        <card type="chart" style="color:lightgrey">
          The graph below describes the appropriateness of module size to map the size of an ontology module.
        </card>
      </div>
      <div class="col-lg-12">
        <card type="chart">
          <template slot="header">
            <h3 class="card-title">Appropriateness of module size</h3>
          </template>
...
export default {
  ...
  created() {
    ...
    for (var i = 0; i < 5; i++) {
      promises.push(
        axios.get(
          "http://localhost:9090/Project_Final/webresources/" +
            label[i] +
            "/metric2"
        )
      );
